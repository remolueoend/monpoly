...
function make_log() {
    ...
    if [ -f "$log" ]; then
        ...
        "./gen_race_avg" $log $length $er $seed
    fi
    echo "${name}"
}
...
function monitor() {
    ...
    #ORACLE-Monpoly
    local oracleCMD="verimon -no_rw -nonewlastts -sig ${WORK_DIR}/synth.sig -formula ${WORK_DIR}/${formula} -log ${logpath}.log -verified > ${verdictpath}_oracle_monpoly"
    ...
    #ORACLE-Dejavu
    local oracleCMD="verimon  -no_rw -nonewlastts -sig ${WORK_DIR}/synth.sig -formula ${WORK_DIR}/${formula_oracle_dejavu} -log ${logpath}.log -verified | cut -d ' ' -f4 | cut -d ')' -f1 | xargs -I J sh -c \"echo 'J+1' | bc -l\" > ${verdictpath}_oracle_dejavu"
