...
let ext_to_m ff neval =
  let el2m linf =
    let mnext = if linf.lnext == NEval.void then -2 else
      NEval.get_index linf.lnext neval in
    (mnext, linf.lnextq)
  in
  let eze2m ezinf =
...
let m_to_ext mf neval = 
  ...
  let ml2e (mnext, mnextq) =
    let lnext =
      if   mnext = -2 then NEval.void
      else                 NEval.get_cell_at_index mnext neval
    in
    {lnext = lnext; lnextq = mnextq}
  in
