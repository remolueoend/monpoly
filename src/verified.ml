...
module Monitor : sig
  ...
  val mstep_e :
    (char list * event_data list) set * nat ->
      (event_data, unit) mstate_ext ->
        (nat * (event_data option) list) set * (event_data, unit) mstate_ext
  ...
  val minit_safe_e : event_data formula -> (event_data, unit) mstate_ext
...
let rec minus_enat x0 n = match x0, n with Enat a, Infinity_enat -> zero_enat
                     ...
                     | Enat a, Enat b -> Enat (minus_nata a b);;
      ...
      (foldr (sup_seta
         ...
         (maps (fun (t, rel) ->
                 (if less_eq_enat (minus_enat (Enat nt) (Enat t)) (right i)
                   then (if less_eq_nat (left i) (minus_nata nt t) then [rel]
                          else [])
                   else []))
           auxa)
         (set_empty
           ((ceq_list (ceq_option _A1)), (ccompare_list (ccompare_option _A2)))
           (of_phantom set_impl_lista)),
        filtera (fun (t, _) -> less_eq_enat (Enat (minus_nata nt t)) (right i))
          auxa));;
