...
|  "safe_letpast p (Exists t \<phi>) = safe_letpast p \<phi>"
|  "safe_letpast p (Agg y \<omega> tys f \<phi>) = safe_letpast p \<phi>"
...
| "safe_formula (Exists t \<phi>) = (safe_formula \<phi> \<and> 0 \<in> fv \<phi>)"
...
lemma safe_formula_induct[consumes 1, case_names Eq_Const Eq_Var1 Eq_Var2 Pred Let LetPast
  ...
  assumes "safe_formula \<phi>"
    ...
    and Exists: "\<And>t \<phi>. safe_formula \<phi> \<Longrightarrow> 0 \<in> fv \<phi> \<Longrightarrow> P \<phi> \<Longrightarrow> P (Exists t \<phi>)" (* any t?*)
...
| "matches v (Exists t \<phi>) e = (\<exists>z. matches (z # v) \<phi> e)"
...
next
  case (Agg y \<omega> tys f \<phi>)
  have "sat \<sigma> V (zs @ v) i \<phi> = sat (map_\<Gamma> (\<lambda>D. D \<inter> E) \<sigma>) V' (zs @ v) i \<phi>" if "length zs = length tys" for zs
