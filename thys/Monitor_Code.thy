...
definition minit_safe_e :: "event_data Formula.formula \<Rightarrow> event_data mstate" where
  [code_unfold]: "minit_safe_e = minit_safe"

definition mstep_e :: "event_data Formula.database \<times> ts \<Rightarrow> event_data mstate \<Rightarrow>
  (nat \<times> event_data tuple) set \<times> event_data mstate" where
