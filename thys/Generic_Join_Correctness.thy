...
lemma correctness:
  assumes "rwf_query n V Q Qn"
      and "card V \<ge> 1"
    shows "z \<in> genericJoin V Q Qn
       \<longleftrightarrow> wf_tuple n V z \<and> (\<forall>(A, X)\<in>Q. restrict A z \<in> X) \<and> (\<forall>(A, X)\<in>Qn. restrict A z \<notin> X)"
  using assms
...
qed
 ...
 text \<open>
 \DefineSnippet{correctness}{
    @{thm [display] correctness}
 }%EndSnippet
 \<close>
...
lemma wrapper_correctness:
  assumes "card Q_pos \<ge>1"
      and "\<forall>(A, X)\<in>(Q_pos \<union> Q_neg). table n A X \<and> wf_set n A"
    shows "z \<in> wrapperGenericJoin Q_pos Q_neg \<longleftrightarrow> wf_tuple n (\<Union>(A, X)\<in>Q_pos. A) z
        \<and> (\<forall>(A, X)\<in>Q_pos. restrict A z \<in> X) \<and> (\<forall>(A, X)\<in>Q_neg. restrict A z \<notin> X)"
...
qed
 ...
 text \<open>
 \DefineSnippet{wrapper_correctness}{
    @{thm [display] wrapper_correctness}
 }%EndSnippet
 \<close>
