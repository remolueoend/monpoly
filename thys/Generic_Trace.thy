(*<*)
theory Generic_Trace
  imports "HOL-Library.Stream"
begin
(*>*)

(*TODO(JS): Replace Trace.thy with this theory and define old 'a trace and 'a prefix
  as abbreviations.*)
...
lemma \<tau>_less_\<tau>: "\<exists>j>i. \<tau> s i < \<tau> s j"
  by (transfer fixing: i) (simp add: sincreasing_def)

lemma ex_le_\<tau>: "\<exists>j\<ge>i. x \<le> \<tau> s j"
proof (transfer fixing: i x)
  fix s :: "('a \<times> nat) stream"
