...
function (sequential) genericJoin :: "vertices \<Rightarrow> 'a query \<Rightarrow> 'a query \<Rightarrow> 'a table" where
  "genericJoin V Q_pos Q_neg =
    (if card V \<le> 1 then
      ...
      let X = {(t,genericJoin J (newQuery J Q_J_pos (I,t)) (newQuery J Q_J_neg (I,t))) | t . t\<in>R_I}
...
termination
 ...
 text \<open>
 \DefineSnippet{generic_join}{
    @{thm [display] genericJoin.simps}
 }%EndSnippet
 \<close>
...
fun wrapperGenericJoin :: "'a query \<Rightarrow> 'a query \<Rightarrow> 'a table" where
 ...
 text \<open>
 \DefineSnippet{wrapper}{
    @{thm [display] wrapperGenericJoin.simps}
 }%EndSnippet
 \<close>
